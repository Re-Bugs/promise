...
public class NotificationValueAPIController {
    ...
    public ResponseEntity<Map<String, Object>> getNotificationValue(@RequestParam String bottleId)
    {
        ...
        if (findUser.isEmpty())
        {
            ...
            log.warn("알림 값 조회(API) - 잘못된 약통코드 : {}", bottleId);
        ...
        log.info("알림값 조회(API) - user PK : {}, 이름 : {}", user.getId(), user.getName());
    ...
    public ResponseEntity<Map<String, String>> setNotificationValue(@RequestParam String bottleId, @RequestParam String value) {
        ...
        {
            response.put("message", "User not found");
            log.warn("알림 값 변경(API) - 잘못된 약통코드 : {}", bottleId);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
        }
        ...
        try
        {
            ...
            log.info("알림값 변경 요청(API) : user PK : {}, 이름 : {}", updateUser.getId(), user.getName());
            return ResponseEntity.status(HttpStatus.OK).body(response);
        }
        catch (IllegalArgumentException e)
        {
            // 잘못된 value 값 처리
            log.warn("잘못된 알림 값 요청(API) - user PK : {}, 이름 : {},  값 : {}", user.getId(), user.getName(), value);
